 ...
 *
 * Run via `pnpm run upgrade`.
 *
 * Mirrors the helper at the plugin repo root (`scripts/build/
 * upgrade-pnpm.cjs`); kept in sync because `@deck-shelves/api`
 * publishes as a standalone npm package and must be self-contained
 * for downstream contributors who clone only the api repo.
 */
'use strict';
